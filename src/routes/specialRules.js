...
router.get('/', authMiddleware, async (req, res) => {
  const { data: rules } = await supabase.schema('fifa2026').from('special_rules').select('*');
  const { data: predictions } = await supabase
    .schema('fifa2026').from('special_rule_predictions').select('*').eq('user_id', req.user.id);
  const predMap = {};
...
router.post('/:id/predict', authMiddleware, async (req, res) => {
  ...
  const { data: rule } = await supabase
    .schema('fifa2026').from('special_rules').select('*').eq('id', req.params.id).single();
  if (!rule) return res.status(404).json({ error: 'Rule not found' });
  if (rule.is_locked) return res.status(403).json({ error: 'Rule is locked' });
  const { data, error } = await supabase
    .schema('fifa2026').from('special_rule_predictions').upsert({
